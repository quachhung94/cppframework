...
void ATask::init()
{
    ...
    // this->demo = std::make_shared<S::Demo>(runenv, "localhost", 8080, "lo");
    std::shared_ptr<CppServer::Asio::Service> _ptrService = this->runenv->getService();
  // get the working io_service(bound to another thread) for queue operation
    // auto _ptrIOService = _ptrService->GetAsioService();
    auto server = std::make_shared<HTTPServerAdapter>(_ptrService, 8080);
    std::string www = "../../CppServer/www/api";
